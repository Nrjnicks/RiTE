...
TimeLimit=Mathf.Round(frm*1.0/60*100)/100;
...
FontD.normal.textColor=Color.green;
GUI.Label(Rect(25,Screen.height-50,300,300),"Game Developed by-Neeraj S.Thakur",FontD);

largeFont.normal.textColor=Color.green;
if(parseInt((TimeLimit%60)/10)==0){
if(parseInt((TimeLimit/60))==0){
largeFont.normal.textColor=Color.red;}
str=parseInt(TimeLimit)/60+":0"+parseInt(TimeLimit)%60;}
else
str=parseInt(TimeLimit)/60+":"+parseInt(TimeLimit)%60;
 GUI.Label(Rect(Screen.width/2,pos.y,size.x,size.y),str,largeFont);
...
if (currentSpeed >= 0){
  rotationAngle = Mathf.Lerp(0,170,speedFactor);
  }
  else {
  rotationAngle = Mathf.Lerp(0,170,-speedFactor);
  }
